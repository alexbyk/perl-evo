...
sub reg_attr ($self, $name, %opts) {
  ...
  my $gen = $self->gen;
  Evo::Internal::Util::monkey_patch $pkg, $name, $gen->gen_attr($name, %opts);
}
...
sub reg_attr_over ($self, $name, %opts) {
  ...
  my $pkg = $self->package;
  my $gen = $self->gen;
  Evo::Internal::Util::monkey_patch_silent $pkg, $name, $gen->gen_attr($name, %opts);
}
...
sub parse_attr ($self, @attr) {
  ...
  croak qq#"default" should be either a code reference or a scalar value#
    if ref $opts{default} && !(reftype($opts{default}) eq 'CODE');
