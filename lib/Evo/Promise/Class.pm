...
sub catch ($self, $cfn) : Public { $self->then(undef, $cfn) }

sub spread ($self, $fn) : Public {
...
sub d_lock_in ($self, $parent) : Public {
...
sub d_fulfill ($self, $v) : Public {

  #assert(!$self->d_settled);
  $self->d_settled(1)->state(FULFILLED)->d_v($v);
}

sub d_reject ($self, $r) : Public {
...
sub d_reject_continue ($self, $reason) : Public {
  $self->d_reject($reason);
  $self->d_traverse;
}

sub d_resolve_continue ($self, $v) : Public {
  $self->d_resolve($v);
